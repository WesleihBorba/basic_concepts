# Goal: Reduce the number of features in dataset
import pandas as pd
...
class PrincipalComponentAnalysis:
    ...
    def __init__(self):
        ...
        self.components, self.pca_data = None, None
    ...
    def number_of_components(self):
        logger.info("Kaiser's Criteria to find number of components, more than 1")
        X = self.normalization_classification.drop(columns={'approved'})
        pca_full = PCA().fit(X)

        eigenvalues = pca_full.explained_variance_
        self.components = len(eigenvalues[eigenvalues > 1])

        logger.debug(f'Number of components: {self.components}')
    ...
    def pca_model(self):
        ...
        cols = [f'PC{i+1}' for i in range(self.components)]
        self.pca_data = pd.DataFrame(pca_array, columns=cols)
        self.pca_data['approved'] = y.values
